...
// Write-side gist commands: push, pull and rm, plus the file helpers they own.
// Split out of gists_cmd.go, which had reached 461 lines (92% of the 500-line
// limit) and could not take new code.
...
func pushUpdate(
	...
	// --clean discards every file you did not name, which is strictly more
	// destructive than overwriting one — yet it used to need no confirmation
	// while overwriting did. Name the casualties and require --force too.
	if clean && len(merged.Dropped) > 0 && !force {
		return fmt.Errorf(
			"[envman] --clean akan menghapus %d file dari gist %q: %s — gunakan --force untuk melanjutkan",
			len(merged.Dropped), existing.Title, strings.Join(merged.Dropped, ", "))
	}
	...
	switch {
	case clean:
		fmt.Fprintf(os.Stderr, "[envman] gist %q diganti — kini %d file", existing.Title, len(merged.Files))
		if len(merged.Dropped) > 0 {
			fmt.Fprintf(os.Stderr, " (%d dihapus: %s)", len(merged.Dropped), strings.Join(merged.Dropped, ", "))
		}
