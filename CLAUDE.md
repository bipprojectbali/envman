...
- `Gist` (id, userId, title, description, files Json `[{filename,content,language}]`, isPublic, tags[]). `isPublic=false` default. Edit/delete: owner atau SUPER_ADMIN.
...
- `GET .../file-health|routes|project-structure|env-map|test-coverage|dependencies|migrations|sessions|schema`
...
**Gists:** `GET .../gists` (sendiri + public; `?limit&cursor&search&filter`) · `POST .../gists` (cap `gist:create`) · `PUT|DELETE .../gists/:id` (owner/SUPER_ADMIN) · `GET .../gists/:id/raw/:filename`. Sidebar gated cap `menu:gists`. **Public (no auth):** `GET /api/public/gists` (`?limit&cursor&search&tags&sort`) · `GET /api/public/gists/:id` (403 jika private) · `.../:id/raw/:filename`.
...
envman sys                                # snapshot kesehatan mesin lokal (host/cpu/mem/disk). --json · --du <dir>
