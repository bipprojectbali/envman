...
**Clipboard:** `GET /api/envman/clip` (decrypt; expired → 404 + auto-delete lazy) · `PUT .../clip` `{content, ttlSeconds?}` (encrypt, upsert overwrite; size > `clipboard_max_kb` → 413; TTL clamp ke `clipboard_max_ttl_hours`) · `DELETE .../clip`. User-level (nempel `userId`, bukan project) — **tidak** di-gate `canWrite` (scratch pribadi, seperti Gist; token read-only tetap boleh set/clear clipboard-nya sendiri). Lihat [Clipboard](#clipboard).
