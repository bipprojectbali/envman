...
const FileEntrySchema = z
  .object({
    filename: z.string().min(1).max(255).describe('Filename including extension (e.g., "deploy.sh", "config.json").'),
...
const FileCreateInputSchema = z
  .object({
    ...
    prefix: z
      ...
      .optional()
      .describe(
        'Prefix slug for CLI reference. Used in canonical "<slug>:<prefix>/<filename>" syntax (e.g., prefix="deploy" → "envman -- bash myapp:deploy/script.sh"). Must be unique per project.',
      ),
