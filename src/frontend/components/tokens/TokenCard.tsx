...
function ScopesRow({ scopes }: { scopes: string[] }) {
  if (scopes.length === 0) {
    return <Badge size="xs" variant="default">semua project</Badge>
  }
  return (
    <>
      {scopes.slice(0, 4).map(s => <ScopeBadge key={s} scope={s} />)}
      {scopes.length > 4 && (
        <HoverCard width={260} shadow="md" withinPortal position="bottom-start">
          <HoverCard.Target>
            <Badge size="xs" variant="default" style={{ cursor: 'pointer' }}>+{scopes.length - 4}</Badge>
          </HoverCard.Target>
          <HoverCard.Dropdown>
            <Stack gap={4}>
              <Text size="xs" c="dimmed">Scope lainnya:</Text>
              <Group gap={4} wrap="wrap">
                {scopes.slice(4).map(s => <ScopeBadge key={s} scope={s} />)}
              </Group>
            </Stack>
          </HoverCard.Dropdown>
        </HoverCard>
    ...
    <Stack gap={6}>
      ...
      {token.scopes.length === 0 && (
        <Text size="xs" c="dimmed">
          Token ini punya akses ke semua project. Ganti <Code fz="xs">myapp:production</Code> dengan project:env yang sesuai.
        </Text>
      )}
    </Stack>
  )
...
export function TokenCard({
  ...
  const statusBadges = (
    <Group gap={4} wrap="nowrap" style={{ flexShrink: 0 }}>
      <Badge size="xs" variant="light" color={accentColor}>{token.canWrite ? 'rw' : 'ro'}</Badge>
      {token.isDisabled && <Badge size="xs" color="gray" variant="filled">off</Badge>}
      {isExpired && <Badge size="xs" color="red" variant="filled">expired</Badge>}
      {expiry === 'soon' && token.expiresAt && (
        <Badge size="xs" color="yellow" variant="light">{daysUntil(token.expiresAt)}d</Badge>
      )}
    </Group>
  )
  ...
  const metaRow = (
    <Group gap={6} wrap="wrap" mt={2}>
      <Tooltip label={token.lastUsedAt ? `Terakhir dipakai ${absoluteTime(token.lastUsedAt)}` : 'Belum pernah dipakai'} withArrow>
        <Group gap={3} style={{ cursor: 'default' }}>
          ...
          <Text size="xs" c="dimmed">
            {token.lastUsedAt
              ...
              : <Text component="span" fs="italic" size="xs" c="dimmed">belum dipakai</Text>}
          </Text>
        </Group>
      </Tooltip>
      ...
      <Tooltip label={`Dibuat ${absoluteTime(token.createdAt)}`} withArrow>
        <Group gap={3} style={{ cursor: 'default' }}>
          ...
          <Text size="xs" c="dimmed">{relativeTime(token.createdAt)}</Text>
        </Group>
      </Tooltip>
      {token.expiresAt && !isExpired && (
        <Tooltip label={absoluteTime(token.expiresAt)}>
          <Text size="xs" c={expiry === 'soon' ? 'yellow' : 'dimmed'} style={{ cursor: 'default' }}>
            · expires {new Date(token.expiresAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
          </Text>
        </Tooltip>
      )}
      {isExpired && token.expiresAt && (
        <Tooltip label={absoluteTime(token.expiresAt)}>
          <Text size="xs" c="red" style={{ cursor: 'default' }}>
            · expired {new Date(token.expiresAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
          </Text>
        </Tooltip>
      )}
    </Group>
  ...
  const actions = (
    // biome-ignore lint/a11y/useKeyWithClickEvents: stop propagation from card click
    <Group gap={2} wrap="nowrap" style={{ flexShrink: 0 }} onClick={e => e.stopPropagation()}>
      ...
      <Menu withinPortal shadow="md" width={160} position="bottom-end">
        ...
        <Menu.Dropdown>
          ...
          <Menu.Item leftSection={<TbPencil size={13} />} onClick={onEdit}>Edit</Menu.Item>
          <Menu.Item leftSection={<TbRefresh size={13} />} color="yellow" onClick={onRotate}>Rotate</Menu.Item>
          <Menu.Divider />
          <Menu.Item leftSection={<TbTrash size={13} />} color="red" onClick={onRevoke}>Revoke</Menu.Item>
        </Menu.Dropdown>
      </Menu>
    </Group>
  )
          ...
          <Group gap="sm" style={{ flex: 1, minWidth: 0 }} wrap="nowrap">
            ...
            <Box style={{ flex: 1, minWidth: 0 }}>
              <Group gap={6} wrap="nowrap" align="center" mb={2}>
                <Text size="sm" fw={700} truncate style={{ flex: 1, minWidth: 0 }}>{token.name}</Text>
                {statusBadges}
              </Group>
              <Group gap={4} wrap="wrap">
  ...
  return (
    ...
    >
      <Box p="sm">
        ...
        <Text fw={700} size="sm" mb={4} truncate>{token.name}</Text>
