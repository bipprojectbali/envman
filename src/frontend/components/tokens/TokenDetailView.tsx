    ...
    <Stack gap="md">
      ...
      <Group gap={6} align="center">
        ...
        <Group gap="xs" wrap="nowrap">
          ...
          <Text size="sm" fw={600} truncate style={{ maxWidth: 320 }}>{token.name}</Text>
        </Group>
      </Group>
      ...
      <Box style={{ opacity: token.isDisabled ? 0.7 : 1 }}>
        <Group justify="space-between" wrap="nowrap" align="flex-start">
          <Group gap="sm" wrap="nowrap" style={{ minWidth: 0 }}>
            ...
            <Box style={{ minWidth: 0 }}>
              <Text fw={700} size="lg" truncate mb={4}>{token.name}</Text>
              <Group gap={6} wrap="wrap">
                <Badge size="sm" variant="light" color={accentColor}>{token.canWrite ? 'read-write' : 'read-only'}</Badge>
                {token.isDisabled && <Badge size="sm" color="gray" variant="filled">disabled</Badge>}
                {isExpired && <Badge size="sm" color="red" variant="filled">expired</Badge>}
                {expiry === 'soon' && token.expiresAt && (
                  <Badge size="sm" color="yellow" variant="light">expires in {daysUntil(token.expiresAt)}d</Badge>
                )}
        ...
        <Group gap="md" wrap="wrap">
          ...
          <Stack gap={2}>
            <Text size="xs" c="dimmed" tt="uppercase" fw={600}>Terakhir dipakai</Text>
            <Tooltip label={token.lastUsedAt ? absoluteTime(token.lastUsedAt) : 'Belum pernah dipakai'} withArrow>
              <Group gap={4} style={{ cursor: 'default' }}>
                ...
                <Text size="sm" fs={token.lastUsedAt ? undefined : 'italic'} c={token.lastUsedAt ? undefined : 'dimmed'}>
          ...
          {token.expiresAt && (
            <Stack gap={2}>
              <Text size="xs" c="dimmed" tt="uppercase" fw={600}>Kedaluwarsa</Text>
              <Tooltip label={absoluteTime(token.expiresAt)} withArrow>
                <Text size="sm" c={isExpired ? 'red' : expiry === 'soon' ? 'yellow' : undefined} style={{ cursor: 'default' }}>
                  {new Date(token.expiresAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
                </Text>
              </Tooltip>
            </Stack>
          )}
      ...
      <Box>
        <Group gap="xs" mb="sm">
          ...
          <Text size="sm" fw={600}>Scope Akses</Text>
          {token.scopes.length > 0 && (
            <Badge size="xs" variant="outline" color="gray">{token.scopes.length} scope</Badge>
          )}
        </Group>
        {token.scopes.length === 0 ? (
          <Group gap="xs">
            <Badge size="sm" variant="light" color="orange">semua project</Badge>
            <Text size="xs" c="dimmed">Token ini punya akses ke semua project yang dimiliki user.</Text>
          </Group>
        ) : (
          <Group gap={6} wrap="wrap">
            {token.scopes.map(s => {
              ...
              if (!linkable) {
                return <Badge key={s} size="sm" variant="default" style={{ fontFamily: 'monospace' }}>{s}</Badge>
              }
              ...
              return (
                <Tooltip key={s} label={`Buka /envmanager/${slug}/${env}`} openDelay={400} withinPortal>
                  <Link to="/envmanager/$slug/$env" params={{ slug, env }} style={{ textDecoration: 'none' }}>
                    <Badge size="sm" variant="dot" color="primary" style={{ fontFamily: 'monospace', cursor: 'pointer' }}>
                      {s}
                    </Badge>
                  </Link>
                </Tooltip>
        ...
        {token.tags.length > 0 && (
          <>
            <Divider my="sm" />
            <Text size="xs" c="dimmed" tt="uppercase" fw={600} mb={6}>Tags</Text>
            <Group gap={6} wrap="wrap">
              {token.tags.map(tag => (
