      ...
      <Stack gap="xs">
        ...
        {tags.length > 0 && (
          <Group gap={4} wrap="wrap">
            {tags.map(t => (
              <Badge key={t} size="xs" variant="light" color="blue" pr={3} rightSection={
...
function FileViewModal({ file, onClose, canManage, onEdit }: {
  file: ProjectFile | null; onClose: () => void; canManage: boolean; onEdit: () => void
}) {
  const [activeFile, setActiveFile] = useState(0)
  ...
  return (
    <Modal
      ...
      title={
        <Group gap="xs" wrap="nowrap" style={{ minWidth: 0 }}>
          ...
          <Text fw={700} lineClamp={1}>{file.title}</Text>
        </Group>
      }
      size="xl"
      fullScreen={isMobile}
    ...
    >
      <Stack gap="sm">
        {file.description && <Text size="sm" c="dimmed">{file.description}</Text>}
        ...
        <Group gap={4} wrap="wrap">
          {file.tags.map(t => <Badge key={t} size="xs" variant="outline" color="gray">{t}</Badge>)}
          <Text size="xs" c="dimmed" ml="auto">oleh {file.author.name} · {relTime(file.updatedAt)}</Text>
        </Group>
        ...
        <Group justify="space-between">
          ...
          {canManage && (
            <Button type="button" size="xs" leftSection={<TbEdit size={13} />} onClick={onEdit}>Edit</Button>
          )}
        </Group>
      </Stack>
    </Modal>
...
export function FilesPanel({ slug, isOwner, myUserId, canEdit }: FilesPanelProps) {
      ...
      children: (
        <Stack gap="sm">
          ...
          <Group justify="flex-end" mt="xs">
            ...
            <Button color="red" leftSection={<TbTrash size={13} />} onClick={() =>
              apiFetch(`/api/envman/projects/${slug}/files/${f.id}`, { method: 'DELETE' })
  ...
  return (
    <Box>
      ...
      >
        {formOpen && <FileForm slug={slug} file={editingFile ?? undefined} onClose={closeForm} />}
      </Modal>
      ...
      {!isLoading && !isError && files.length === 0 && (
        <Card withBorder p="xl" ta="center" style={{ borderStyle: 'dashed' }}>
          ...
          <Text size="sm" c="dimmed" mb="md" maw={400} mx="auto">
            Simpan snippets, config files, script, atau template untuk project ini. Mendukung multi-file dan preview Markdown.
          </Text>
          {canEdit && (
