...
interface ProjectDetailHeaderProps {
  slug: string
  project: { name: string; description?: string; tags?: string[]; myRole?: string; members?: unknown[]; environments?: unknown[]; createdAt?: string } | undefined
...
export function ProjectDetailHeader({ slug, project, envs, isLoading, isError, error, refetch, myRole, totalVars, memberCount, projectTags }: ProjectDetailHeaderProps) {
  return (
    <>
      <Group mb="md" gap={6} wrap="nowrap" align="center">
        <Button variant="subtle" size="xs" px={8} color="gray" component={Link} to="/envmanager" leftSection={<TbArrowLeft size={12} />} styles={{ root: { fontWeight: 400 } }}>
          Projects
        </Button>
      ...
      {isLoading ? (
        ...
        <Box p="xl" ta="center" mb="md" style={{ border: '1px solid var(--mantine-color-red-5)' }}>
          ...
          <Group justify="center" gap="xs">
            <Button size="xs" variant="subtle" color="gray" component={Link} to="/envmanager">Kembali ke daftar</Button>
        ...
        project && (
          <Box p={{ base: 'sm', sm: 'md' }} mb="lg" style={{ border: '1px solid var(--mantine-color-default-border)', borderRadius: 'var(--mantine-radius-md)' }}>
            <Group gap="sm" wrap="nowrap" align="flex-start">
              <ThemeIcon size={48} radius="lg" variant="light" color={roleColor[myRole as keyof typeof roleColor] ?? 'gray'} style={{ flexShrink: 0 }}>
                <TbFolders size={22} />
              </ThemeIcon>
              <Box style={{ flex: 1, minWidth: 0 }}>
                <Group gap="xs" mb={4} wrap="wrap" align="center">
                  <Text fw={800} size="xl" lh={1.2} style={{ wordBreak: 'break-word' }}>{project.name}</Text>
                  <Group gap={6} wrap="nowrap" style={{ flexShrink: 0 }}>
                    <Code fz="xs">{slug}</Code>
                    <Badge size="sm" variant="light" color={roleColor[myRole as keyof typeof roleColor] ?? 'gray'}>{myRole}</Badge>
                  </Group>
                </Group>

                <Text size="sm" lh={1.6} mb="xs" c={project.description ? undefined : 'dimmed'} fs={project.description ? undefined : 'italic'}>
                ...
                <Group gap="xs" wrap="wrap" pt="xs" mt={4} style={{ borderTop: '1px solid var(--mantine-color-default-border)' }}>
