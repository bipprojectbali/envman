  ...
  return (
    <Box p="sm" style={{ overflow: 'visible', border: '1px solid var(--mantine-color-default-border)', borderRadius: 'var(--mantine-radius-md)' }}>
        ...
        <Group gap="xs" wrap="nowrap" style={{ minWidth: 0, flex: 1 }}>
          ...
          <Box style={{ minWidth: 0, flex: 1 }}>
            ...
            <Text size="xs" c="dimmed" truncate>
              ...
              {project.environments.length > 0 && ` · ${project.environments.length} env${project.environments.length !== 1 ? 's' : ''}`}
            </Text>
          </Box>
        </Group>
        <Group gap={4} wrap="nowrap">
          {hasRestricted && (
            <Tooltip label="Ada env yang di-deny" withArrow>
              <Badge size="xs" color="red" variant="dot">restricted</Badge>
            </Tooltip>
          )}
          {!hasRestricted && hasOverride && (
            <Tooltip label="Ada env dengan role override" withArrow>
              <Badge size="xs" color="orange" variant="dot">override</Badge>
            </Tooltip>
          )}
        </Group>
      </Group>
      ...
      <Group gap="xs" mb={6}>
        ...
        <Text size="xs" fw={600} c="dimmed" tt="uppercase">Default role</Text>
      </Group>
      <Chip.Group
      ...
      {project.environments.length > 0 && (
        <>
          <Divider my="sm" label={<Text size="xs" c="dimmed">Per-env override</Text>} labelPosition="left" />
          <Stack gap={4}>
              ...
              return (
                <Group key={env.name} gap="xs" wrap="nowrap" align="center">
                  ...
                  {env.envRole === 'denied' ? (
                    ...
                    <Text size="xs" c="dimmed" style={{ flexShrink: 0 }}>inherit</Text>
                  ) : env.effectiveRole ? (
