...
export const DESTRUCTIVE_CAPABILITIES = ['stack:prune'] as const
      ...
      <Stack gap="md">
        {CAPABILITY_GROUPS.map((group) => {
          ...
          return (
            <Box
              key={group.label}
              style={{ overflow: 'hidden', border: '1px solid var(--mantine-color-default-border)', borderRadius: 'var(--mantine-radius-md)' }}
            >
              <Box
                p="sm"
                style={{ background: `var(--mantine-color-${group.color}-light)`, borderBottom: '1px solid var(--mantine-color-default-border)' }}
              >
                <Group justify="space-between" wrap="nowrap" gap="xs">
                  <Group gap="xs" wrap="nowrap" style={{ minWidth: 0, flex: 1 }}>
                    ...
                    <Box style={{ minWidth: 0, flex: 1 }}>
                      <Text size="sm" fw={700} c={group.color} truncate>{group.label}</Text>
                      <Text size="xs" c="dimmed" truncate>{group.description}</Text>
                    </Box>
                  </Group>
                  <Group gap="xs" wrap="nowrap" style={{ flexShrink: 0 }}>
                    <Badge size="sm" color={group.color} variant="filled">{groupSelected}/{groupTotal}</Badge>
              ...
              <Stack gap="xs" p="sm">
                {group.items.map((cap) => (
                  <Box
                    ...
                    style={{
                      borderRadius: 6,
                      background: selected.includes(cap.value) ? `var(--mantine-color-${group.color}-light)` : 'transparent',
