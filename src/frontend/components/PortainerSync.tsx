...
import { hasCapability, useSession } from '../hooks/useAuth'
...
export function PortainerSync({ slug, env, canEdit, secretCount, onSetupOpen }: Props) {
  ...
  // exec env-level memanggil endpoint connection-scoped (/connections/:id/exec) yang
  // butuh stack:exec — tanpa fallback role. Gate tombolnya terpisah dari canEdit.
  const canExec = isSuperAdmin || hasCapability(session?.user, 'stack:exec')
  ...
  return (
    <>
      ...
      <PortainerLogsModal
        ...
        containers={containers}
        statusFetching={statusFetching}
        initialContainerId={logsInitialContainerId}
      />
      <PortainerExecModal
        opened={execOpen}
        onClose={closeExec}
