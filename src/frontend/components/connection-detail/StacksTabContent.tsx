import { ActionIcon, Alert, Badge, Box, Button, Group, Pagination, SimpleGrid, Stack, Text, Tooltip } from '@mantine/core'
...
export function StacksTabContent({ stacks, filteredStacks, stackView, setStackView, totalPages, page, setPage, search, setSearch, filterStatus, setFilterStatus, filterType, setFilterType, filterLinked, setFilterLinked, hasFilter, stackStatusMap, containerStatsMap, canMutate, canOperate, repull, recreate, restartContainer, confirmRepull, confirmRecreate, confirmRestartContainer, onOpenCompose, onOpenLogs, onOpenExec }: Props) {
            ...
            {pagedStacks.map((stack) => {
              ...
              return (
                <StackItem key={stack.id} stack={stack} view="grid" stackContainers={stackContainers} stackFetching={stackFetching} containerStatsMap={containerStatsMap} canMutate={canMutate} canOperate={canOperate}
                  ...
                  onOpenCompose={onOpenCompose} onOpenLogs={onOpenLogs} onOpenExec={onOpenExec} />
              )
            })}
          </SimpleGrid>
