import { ActionIcon, Badge, Box, Code, CopyButton, Group, Pagination, Table, Text, Tooltip } from '@mantine/core'
import {
  TbCheck,
  TbClipboard,
  TbCopy,
      ...
      <Table highlightOnHover verticalSpacing="xs" horizontalSpacing="sm">
        <Table.Thead style={{ background: 'var(--mantine-color-default-hover)' }}>
          <Table.Tr>
            ...
            <Table.Th w={260} style={thStyle}>
              Key
            </Table.Th>
            <Table.Th style={thStyle}>Value</Table.Th>
            <Table.Th w={90} style={thStyle}>
              Diperbarui
            </Table.Th>
            <Table.Th w={canEdit ? 130 : 50} style={thStyle}>
              Aksi
            </Table.Th>
          </Table.Tr>
        </Table.Thead>
          ...
          {filteredVars.map((v) => {
            ...
            return (
              ...
              >
                ...
                >
                  {(() => {
                    // Values are hidden by default; showValue() decides visibility
                    // (secrets need per-value reveal; plain also honor the global flag).
                    ...
                    return (
                      <Group gap="xs" wrap="nowrap">
                        ...
                        {maskable && v.value !== '***' && (
                          <Tooltip label={shown ? 'Sembunyikan' : 'Tampilkan'}>
                            <ActionIcon
                              size="xs"
                              variant="subtle"
                            ...
                            >
                              {shown ? <TbEyeOff size={12} /> : <TbEye size={12} />}
                            </ActionIcon>
                          </Tooltip>
                        )}
                      </Group>
