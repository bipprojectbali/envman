...
export function MarkdownRenderer({ children, fontSize = 14 }: MarkdownRendererProps) {
  // useComputedColorScheme meresolusi "auto" → "light"/"dark" sesuai preferensi
  // sistem; useMantineColorScheme bisa return "auto" sehingga tema syntax salah.
        ...
        components={{
          code({ className, children: codeChildren, ...rest }) {
            ...
            return lang ? (
              ...
              <pre style={{ fontSize: 13, maxWidth: '100%', overflowX: 'auto' }}>{String(codeChildren).trimEnd()}</pre>
            )
          },
