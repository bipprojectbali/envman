...
function CronBuilder({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  function emit(
    updates: Partial<{ freq: CronFreq; hour: string; min: string; dom: string; dow: string[]; rawCron: string }>,
  ) {
          ...
          <Table.Tbody>
            {backups.map((b) => (
              <Table.Tr key={b.id} bg={selected.has(b.id) ? 'var(--mantine-color-blue-light)' : undefined}>
                <Table.Td>
                  <Checkbox checked={selected.has(b.id)} onChange={() => toggleOne(b.id)} size="xs" />
                </Table.Td>
                <Table.Td>
