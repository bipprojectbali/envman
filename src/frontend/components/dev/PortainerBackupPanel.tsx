...
function buildCron(
  ...
  switch (freq) {
    ...
    case "daily":
      return `${m} ${h} * * *`;
    case "weekly":
      return `${m} ${h} * * ${dow.length ? [...dow].sort((a, b) => +a - +b).join(",") : "*"}`;
    case "monthly":
      return `${m} ${h} ${dom || "1"} * *`;
    default:
      return "0 2 * * *";
...
function cronLabel(
  ...
  switch (freq) {
    case "hourly":
      return `Tiap jam di menit :${String(+min).padStart(2, "0")}`;
    case "daily":
      return `Tiap hari pukul ${t}`;
    case "weekly":
      return `Tiap ${dow.map((d) => dayNames[+d] ?? d).join(", ")} pukul ${t}`;
    case "monthly":
      return `Tiap bulan tanggal ${dom} pukul ${t}`;
    default:
      return "";
  }
}
...
function CronBuilder({
  ...
  const [hour, setHour] = useState(
    initParts.hour !== "*" ? initParts.hour : "2",
  );
  const [min, setMin] = useState(
    VALID_MINUTES.has(initParts.min) ? initParts.min : "0",
  );
  const [dom, setDom] = useState(initParts.dom !== "*" ? initParts.dom : "1");
  const [dow, setDow] = useState<string[]>(
    initParts.dow.length ? initParts.dow : ["1"],
  );
  ...
  const derived =
    freq === "custom" ? rawCron : buildCron(freq, hour, min, dom, dow);
          ...
          >
            <Group gap="xs">
              {DOW_LABELS.map((lbl, i) => (
                <Chip key={lbl} value={String(i)} size="sm">
                  {lbl}
                </Chip>
              ))}
            </Group>
      ...
      {freq === "custom" && (
        <TextInput
          ...
          onChange={(e) => {
            ...
            emit({ rawCron: e.currentTarget.value });
          }}
        />
      )}
            ...
            <Button
              ...
              onClick={() =>
                modals.openConfirmModal({
                  title: "Hapus jadwal?",
                  children: (
                    <Text size="sm">Backup terjadwal akan dihentikan.</Text>
                  ),
...
function HistoryTab({ connId }: { connId: string }) {
  ...
  const listQ = useQuery<{
    ...
    queryFn: () => {
      ...
      return apiFetch(
        `/api/envman/portainer/connections/${connId}/backups?${qs}`,
      );
    },
  });
  ...
  const allChecked =
    backups.length > 0 && backups.every((b) => selected.has(b.id));
  ...
  const confirmDelete = (ids: string[]) =>
    modals.openConfirmModal({
      title: "Hapus backup?",
      children: (
        <Text size="sm">{ids.length} backup akan dihapus permanen.</Text>
      ),
      labels: { confirm: "Hapus", cancel: "Batal" },
      confirmProps: { color: "red" },
      ...
      {activeConn ? (
        <Tabs defaultValue="backup" variant="outline">
          <Tabs.List>
            <Tabs.Tab
              value="backup"
              leftSection={<TbDatabaseExport size={14} />}
            >
              Backup Sekarang
            </Tabs.Tab>
            <Tabs.Tab
        ...
        !connectionsQ.isLoading && (
          <Text size="sm" c="dimmed">
            Belum ada Portainer connection. Buat di halaman Connections terlebih
            dahulu.
          </Text>
        )
      )}
