...
import { FlowNode, METHOD_COLORS, storageKey } from './shared-node-types'
  ...
  return (
    ...
    >
      ...
      <Group gap={8}>
        ...
        {data.avgDuration > 0 && (
          <Text size="xs" c="dimmed">{data.avgDuration}ms avg</Text>
        )}
      </Group>
    </div>
  )
  ...
  return (
    <>
      <Group px="md" pb="xs" gap="sm">
        ...
        {errorCount > 0 && (
          <Badge size="sm" color="red" variant="light">{errorCount} errors</Badge>
        )}
        <ActionIcon
