...
import { storageKey } from './shared-node-types'
  ...
  return (
    ...
    >
      ...
      <Group gap={4} mt={4}>
        ...
        {data.userBlocked && (
          <Badge size="xs" color="red" variant="filled">BLOCKED</Badge>
        )}
...
function RoleAccessNode({ data }: { data: { label: string; routes: string[]; color: string; count: number } }) {
  return (
    ...
    >
      ...
      <Stack gap={2} mt={4}>
        {data.routes.map((r) => (
          <Text key={r} size="xs" c="dimmed" ff="monospace">{r}</Text>
        ))}
      </Stack>
    </div>
  )
    ...
    <>
      <Group px="md" pb="xs" gap="sm">
        ...
        <Tooltip label="Reload">
          <ActionIcon variant="subtle" size="sm" loading={isFetching} onClick={() => qc.invalidateQueries({ queryKey: ['admin', 'sessions'] })}>
            <TbRefresh size={16} />
          </ActionIcon>
        </Tooltip>
      </Group>
