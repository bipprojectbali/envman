...
function ProjectDetailPage() {
  ...
  const setTab = (t: string) =>
    navigate({
      ...
      params: { slug },
      search: {
        tab: t as 'environments' | 'notes' | 'aliases' | 'files' | 'members' | 'storage',
        fileId,
        fileNew,
        viewFileId,
        aliasId,
        ...
        <Tabs value={activeTab} onChange={(v) => setTab(v ?? 'environments')} variant="pills" color="gray">
          <Tabs.List mb="lg">
            ...
            {canView(storageRole) && (
              <Tabs.Tab
                value="storage"
                leftSection={<TbFolderOpen size={13} />}
                rightSection={
          ...
          {canView(notesRole) && (
            <Tabs.Panel value="notes">
              <Paper withBorder p="md" radius="md">
                <NotesPanel
                  slug={slug}
                  canEdit={canEditSec(notesRole)}
                  isOwner={notesRole === 'OWNER'}
                  myUserId={myUserId ?? ''}
                />
              </Paper>
            </Tabs.Panel>
          ...
          {canView(filesRole) && (
            <Tabs.Panel value="files">
              <Paper withBorder p="md" radius="md">
                <FilesPanel
                  slug={slug}
                  isOwner={filesRole === 'OWNER'}
                  myUserId={myUserId ?? ''}
                  canEdit={canEditSec(filesRole)}
                />
              </Paper>
            </Tabs.Panel>
          )}
          ...
          {canView(storageRole) && (
            <Tabs.Panel value="storage">
              <Paper withBorder p="md" radius="md">
                <StoragePanel
                  slug={slug}
                  isOwner={storageRole === 'OWNER'}
                  canEdit={canEditSec(storageRole)}
