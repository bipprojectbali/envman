...
function ConnectionDetailPage() {
  ...
  const canExec = isSuperAdmin || hasCapability(user, 'stack:exec')
  const canPower = isSuperAdmin || hasCapability(user, 'stack:power')
  const canDeploy = isSuperAdmin || hasCapability(user, 'stack:deploy')
  ...
  const {
    ...
    networksFetching,
    refetchNetworks,
  } = useConnectionDetail(id, { activeTab })
  ...
  const {
    ...
    recreate,
    confirmRecreate,
  } = useStackMutations({ id, composeStack, composeContent, setComposeEditing })

  const { pruneImages, confirmPruneImages, pruneContainers, pruneVolumes, pruneNetworks } = useCleanupMutations({
    id,
    cleanupEndpointId,
    imagesData,
    refetchImages,
  ...
  const {
    ...
    execOpen,
    closeExec,
    openExecForContainer,
    execCommand,
    setExecCommand,
    execHistory,
    setExecHistory,
    execQuickCommands,
    setExecQuickCommands,
    execShowQuickAdd,
    setExecShowQuickAdd,
    execNewQuickLabel,
    ...
    <Box>
      ...
      {activeTab === 'stacks' && (
        <StacksTabContent
          ...
          confirmRepull={confirmRepull}
          confirmRecreate={confirmRecreate}
          confirmRestartContainer={confirmRestartContainer}
          onOpenCompose={handleOpenCompose}
          onOpenLogs={handleOpenLogs}
          onOpenExec={openExecForContainer}
        />
      )}
      ...
      {activeTab === 'maintenance' && (
        <MaintenanceTab
          ...
          volumesFetching={volumesFetching}
          refetchVolumes={refetchVolumes}
          networksData={networksData}
          networksFetching={networksFetching}
          refetchNetworks={refetchNetworks}
          pruneImages={pruneImages}
          confirmPruneImages={confirmPruneImages}
