  ...
  const confirmRestartContainer = (stack: StackInfo, containerId: string, containerName: string) =>
    modals.openConfirmModal({
      ...
      children: (
        <Stack gap="xs">
          <Text size="sm">
            Restart container <strong>{containerName}</strong>?
          </Text>
  ...
  const confirmSaveCompose = () =>
    modals.openConfirmModal({
      ...
      children: (
        <Stack gap="xs">
          <Text size="sm">
            Simpan perubahan ke stack <strong>{composeStack?.name}</strong>?
          </Text>
          <Alert color="orange" icon={<TbAlertTriangle size={14} />} p="xs">
            <Text size="xs">
  ...
  const confirmRepull = (stack: StackInfo) =>
    modals.openConfirmModal({
      ...
      children: (
        <Stack gap="xs">
          <Text size="sm">
            Pull image terbaru untuk stack <strong>{stack.name}</strong>?
          </Text>
          <Alert color="orange" icon={<TbAlertTriangle size={14} />} p="xs">
            <Text size="xs">
  ...
  if (!connection) {
    return (
      <Alert color="red" icon={<TbAlertTriangle size={16} />}>
        ...
        <Button size="xs" mt="xs" component={Link} to="/envmanager/connections">
          ← Kembali
        </Button>
      </Alert>
    )
      ...
      <Tabs value={activeTab} onChange={(v) => setActiveTab(v ?? 'stacks')} mb="sm">
        <Tabs.List>
          <Tabs.Tab value="stacks" leftSection={<TbServer size={14} />}>
            Stacks
            {stacks.length > 0 && (
              <Badge size="xs" variant="light" color="primary" ml="xs">
                {stacks.length}
              </Badge>
            )}
          </Tabs.Tab>
                          ...
                          {stackFetching && stackContainers.length === 0 ? (
                            ...
                            stackContainers.map((c) => (
                              <Box
                                ...
                                style={{
                                  borderRadius: 'var(--mantine-radius-md)',
                                  border: `1px solid ${stateColor[c.state] ? `var(--mantine-color-${stateColor[c.state]}-3)` : 'var(--mantine-color-default-border)'}`,
                                  cursor: 'pointer',
                                }}
                                onClick={() => {
                                  setLogsStack(stack)
          ...
          >
            ...
            <Group justify="space-between" mb="md" wrap="wrap" gap="xs">
              <Box>
                ...
                <Text size="xs" c="dimmed">
                  Images yang tidak dipakai container manapun di host ini
                </Text>
              </Box>
              <Group gap="xs">
                {imagesData && (
                    ...
                    <Table.Tbody>
                      {imagesData.images.map((img: any) => (
                        <Table.Tr key={img.id}>
                          ...
                          <Table.Td>
                            <Text fz="xs">{fmtBytes(img.size)}</Text>
                          </Table.Td>
                        </Table.Tr>
                      ))}
                    </Table.Tbody>
            ...
            <Group justify="space-between" mb="sm" wrap="wrap" gap="xs">
              <Box>
                <Text fw={600} size="sm" mb={2}>
                  Stopped / Dead Containers
                </Text>
                <Text size="xs" c="dimmed">
                  Container yang sudah berhenti dan belum dihapus
                </Text>
              </Box>
              <Group gap="xs">
                {containersData && (
            ...
            <Group justify="space-between" mb="sm" wrap="wrap" gap="xs">
              <Box>
                ...
                <Text size="xs" c="dimmed">
                  Volume yang tidak dipakai container manapun
                </Text>
              </Box>
              <Group gap="xs">
                {volumesData && (
                  ...
                  <Table fz="xs" horizontalSpacing="sm" verticalSpacing={4} highlightOnHover>
                    ...
                    <Table.Tbody>
                      {volumesData.volumes.map((v: any) => (
                        <Table.Tr key={v.name}>
                          <Table.Td>
                            <Code fz={10}>{v.name}</Code>
                          </Table.Td>
                          <Table.Td>
                            <Text fz="xs">{v.driver}</Text>
                          </Table.Td>
                          <Table.Td>
                            <Text fz="xs">{relTime(v.createdAt)}</Text>
                          </Table.Td>
                        </Table.Tr>
                      ))}
                    </Table.Tbody>
            ...
            <Group justify="space-between" mb="sm" wrap="wrap" gap="xs">
              <Box>
                ...
                <Text size="xs" c="dimmed">
                  Network Docker yang tidak dipakai container manapun
                </Text>
              </Box>
              <Group gap="xs">
                {networksData && (
                  ...
                  <Table fz="xs" horizontalSpacing="sm" verticalSpacing={4} highlightOnHover>
                    ...
                    <Table.Tbody>
                      {networksData.networks.map((n: any) => (
                        <Table.Tr key={n.id}>
                          <Table.Td>
                            <Code fz={10}>{n.id}</Code>
                          </Table.Td>
                          <Table.Td>
                            <Text fz="xs">{n.name}</Text>
                          </Table.Td>
                          <Table.Td>
                            <Text fz="xs">{n.driver}</Text>
                          </Table.Td>
                          <Table.Td>
                            <Text fz="xs">{n.scope}</Text>
                          </Table.Td>
                        </Table.Tr>
                      ))}
                    </Table.Tbody>
