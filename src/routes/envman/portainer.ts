...
export const portainerRouter = new Elysia()
  ...
  // Container exec (non-interactive)
  .post('/api/envman/portainer/connections/:id/exec', async ({ request, params, set }) => {
    ...
    if (!body?.containerId || !body?.endpointId || !body?.command) {
      set.status = 400; return { error: 'containerId, endpointId, command required' }
    }
    ...
    try {
      ...
      const createRes = await fetch(
        `${url}/api/endpoints/${endpointId}/docker/containers/${containerId}/exec`,
        {
          method: 'POST',
          headers: { 'X-API-Key': conn.apiToken, 'Content-Type': 'application/json' },
          body: JSON.stringify({ AttachStdin: false, AttachStdout: true, AttachStderr: true, Tty: false, Cmd: ['/bin/sh', '-c', command] }),
        },
      )
      ...
      // Step 2: Start exec — returns multiplexed Docker stream
      const startRes = await fetch(
        `${url}/api/endpoints/${endpointId}/docker/exec/${execId}/start`,
        {
          method: 'POST',
          headers: { 'X-API-Key': conn.apiToken, 'Content-Type': 'application/json' },
      ...
      // Step 3: Parse Docker multiplexed stream (same format as container logs)
      ...
      try {
        const inspectRes = await fetch(`${url}/api/endpoints/${endpointId}/docker/exec/${execId}/json`, { headers: { 'X-API-Key': conn.apiToken } })
        if (inspectRes.ok) {
      ...
      appLog('info', `[portainer:exec] connection=${conn.name} container=${containerId.slice(0, 12)} cmd="${command.slice(0, 60)}" exit=${exitCode}`)
      return { stdout: stdoutLines, stderr: stderrLines, exitCode }
    } catch (e) {
      set.status = 500; return { error: `Exec failed: ${e instanceof Error ? e.message : String(e)}` }
    }
  })
